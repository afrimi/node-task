...
class TransactionProcessor {
    ...
    processTransaction(transaction, transactions) {
        ...
        switch (transaction.type) {
            case TransactionTypes.CASH_IN: {
                let cashIn = new CashIn(this.config.cashIn);
            ...
            case TransactionTypes.CASH_OUT: {
                switch (transaction.user_type) {
                    case UserTypes.NATURAL: {
                        let cashOutNatural = new CashOutNatural(this.config.cashOut.natural);
                        const filteredTransactions = this.filterCashOutWeekLimit(transaction, transactions);
                        const totalWeekAmount = this.calculateSum(filteredTransactions);
                        calculatedCommission = cashOutNatural.process(transaction, totalWeekAmount);
                        break;
                    }

                    case UserTypes.JURIDICAL: {
                        const cashOutJuridical = new CashOutJuridical(this.config.cashOut.legal);
                        calculatedCommission = cashOutJuridical.process(transaction.operation.amount);
                        break;
                    }
                }
            }
    ...
    calculateSum(filteredTransactions) {
        return filteredTransactions.reduce((acc, curr) => acc + curr.operation.amount, 0);
    }
